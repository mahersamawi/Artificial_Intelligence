...
def posIsValid(pos):
	...
	return (pos.getRow() >= 0 and pos.getCol() >= 0 and pos.getRow() < numRows and pos.getCol() < numCols )
...
def BFS(maze_list, start_pos, dot):
	...
	while not (frontier.empty()):
		...
		if(frontier.empty()):
			return -1
		node = frontier.get()
		explored.append(node.getPosition())
		children = getChildren(node.getPosition(), maze_list)
		for loc in children:
			child = Node(loc, node, 1 + node.getPathCost())
			# if not (child.getPosition() in explored or child in frontier.queue):
			if (not (isInExplored(child.getPosition(), explored)) or not (child in frontier.queue)):
				print("CHILD: " + child.getPosition().printPos() + " DOT: " + dot.getPosition().printPos())
				if (child.getPosition().__eq__(dot.getPosition())):
					print("SUCCESS")
					return 1 + node.getPathCost()
				frontier.put(child)
...
def main():
	...
	for dot in dot_list:
		print("Dot row: " + str(dot.getPosition().getRow()) + " Dot col: " + str(dot.getPosition().getCol()))

	print("Start row: " + str(start_pos.getRow()) + " Start col: " + str(start_pos.getCol()))
